...
- [关闭 `debug` 后丢失开发期热重载与堆栈，影响排障效率] → 通过环境变量保留显式开启路径（仅供本机排障），verbose 日志写文件不入响应；文档说明如何临时开启。
...
- 是否在本次一并把 `config.json` 与 TLS 证书物理迁移出 `BASE_DIR`（更强加固，但牵动发布打包与脚本）？倾向作为后续增量，本次先保证"不可服务"。
- 是否需要为反代场景提供自动告警阈值（如检测大量来自 `127.0.0.1` 的非本地特征请求），还是仅以文档 + 手动开关为准？倾向后者，避免误报。
