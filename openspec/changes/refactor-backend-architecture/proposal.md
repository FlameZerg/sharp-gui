...
当前后端入口 `app.py` 已增长到约 2800 行，集中承载 Flask 应用创建、局域网门禁、模型图库、本地照片图库、任务队列、模型格式转换、静态文件服务、设置管理、重启与导出 HTML 等职责。随着照片图库、LAN 门禁、SPZ 导出等能力继续增加，单文件结构已经让阅读、变更定位、权限边界审查和后续扩展变得困难。
...
- 按职责拆分后端模块：
  ...
  - 服务层：模型图库、照片图库、任务队列、PLY/SPZ/Splat 转换、独立 HTML 导出、文件夹选择。
...
- 风险区域：
  ...
  - 照片图库索引锁、路径归一化、缩略图缓存和批量 ZIP 下载临时文件清理。
