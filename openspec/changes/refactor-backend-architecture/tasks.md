...
- [ ] 1.1 记录当前 `app.py` 的 Flask route map，保存为重构前端点基线，覆盖页面、认证、模型图库、照片图库、照片相册上传、任务、设置、静态文件和导出路由。
- [ ] 1.2 梳理当前 `app.py` 的全局状态与副作用清单，包括 workspace 路径、`photo_index_lock`、`PHOTO_MAX_UPLOAD_BATCH`、`task_status`、`running_processes`、后台线程启动和 verbose 日志。
...
- [ ] 1.5 补充安全 pytest，覆盖门禁开/关、owner-only 拒绝远程、远程生成条件、照片相册上传条件写入和转发头不能提升 owner。
- [ ] 1.6 补充静态文件安全 pytest，覆盖模型文件允许访问、敏感文件和路径穿越请求被拒绝。
- [ ] 1.7 补充高风险纯函数和服务层 pytest，覆盖配置 normalize、路径归一化、photo id 解析、照片上传文件名净化/扩展名白名单/无效图片清理、任务状态操作和无需真实推理的入队/取消逻辑。
...
- [ ] 4.2 调整 `get_required_access_level()` 为集中权限矩阵函数，保持所有现有 Public、Unlocked、Owner、Conditional 路径语义不变，并显式保留 `POST /api/photo-albums/<album_id>/uploads` 在门禁开启时 Unlocked、门禁关闭时 Owner 的条件写入语义。
...
- [ ] 4.6 运行安全 pytest，确认门禁、CORS、owner-only、远程生成、相册上传条件写入和敏感资源保护未回退。
...
- [ ] 5.5 迁移照片图库逻辑到 `services/photo_gallery.py` 和 `routes/photo_gallery.py`，覆盖相册配置、扫描、分页排序、缩略图、原图、相册上传、ZIP 下载和照片转 3D。
- [ ] 5.5.1 将 `make_unique_photo_upload_filename()`、`is_valid_uploaded_photo()`、上传批量上限、保存失败清理和上传后扫描刷新迁移到照片图库服务，route 层仅保留 form-data 读取与响应封装。
...
- [ ] 6.2 更新 `.agents/rules/backend-guide.md`，说明 app factory、routes、services、security、config、paths、TaskManager 的职责边界，并补充照片相册上传端点的权限与文件安全规则。
...
- [ ] 7.2 运行 pytest 后端测试，覆盖认证状态、模型图库、照片图库、照片相册上传、任务状态、设置读取、静态文件、导出关键路径和 app import 不启动 worker 的行为。
- [ ] 7.3 运行安全 pytest，确认敏感文件、路径穿越、远程 owner-only、远程生成条件、远程相册上传条件和转发头场景均符合规格。
