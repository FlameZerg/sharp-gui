...
#### Scenario: 底部浏览导航视觉参考验收
- **WHEN** 模型资产结果超过单屏容量
- **THEN** 网格底部 SHALL 提供接近示例图的分页、翻页或批次加载导航
- **AND** 桌面布局 SHALL 提供每页数量或批次大小选择
- **AND** 当前页、相邻页、前后翻页或加载边界 SHALL 清晰可见
...
系统 SHALL 支持按来源、格式、标签或媒体关联筛选模型资产，按修改时间、创建时间、名称、大小等常见字段排序，并支持网格密度调整和分页或游标式增量加载。
