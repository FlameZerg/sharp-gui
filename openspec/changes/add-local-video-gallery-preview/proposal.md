...
- 支持常见 SDR 视频的浏览与预览，优先使用浏览器原生硬件解码能力；播放 URL 使用短期签名票据和带文件名后缀的路径式地址，提升移动端原生播放器接管后的兼容性。
- 对无法播放的视频提供项目风格一致的玻璃态错误提示和下载入口，并明确浏览器接管失败时建议下载或更换浏览器。
...
- 权限与安全：`backend/security/access_control.py` 需要覆盖新增媒体预览、封面、下载、路径式播放票据和可选处理端点。
