...
#### Scenario: 实验引擎不可用但稳定路线可用
- **WHEN** 实验初始化依赖不可用且稳定 3DGS 路线可用
- **THEN** 设置 UI SHALL 将实验引擎描述为可选增强而不是必需依赖
- **AND** UI SHOULD 禁用强制实验引擎选择或明确提示无法运行
- **AND** Auto 和 Stable 路线 SHALL 仍可用于创建视频重建任务
...
- **THEN** UI SHALL 说明 `自动`、`稳定` 和 `实验` 的差异
- **AND** UI SHALL 说明实验路线未就绪不会阻止自动或稳定路线运行
