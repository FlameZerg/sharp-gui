...
- 新增质量选择：`快速预览`、`高质量`、`极致` 三个默认预设，以及 `自定义` 单次任务参数入口。默认面向 12GB 显存笔记本 GPU 控制帧预算和训练资源，自定义允许用户按素材长度和目标效果调整关键参数。
...
- 已将固定 `长视频` 档调整为 `自定义` 入口：用户可按单次任务填写目标帧数、训练迭代、输入下采样、COLMAP 匹配方式和图像缓存。长视频可从约 600 帧、35k 迭代、2x 输入下采样、sequential 匹配和 CPU 图像缓存起步，但不再把这组参数包装成通用预设。
