...
**原因**：匹配只影响 COLMAP 阶段中的“特征匹配”子步骤，不影响占总耗时约 80% 的训练；实测 high 档总耗时约 52 分钟、COLMAP 阶段约 8 分钟，换 exhaustive 预计总时长增加约 5~10%，换取显著更高的配准成功率与完整性，性价比高。extreme 帧数翻倍会让 exhaustive 的匹配对数翻 4 倍，但用户实测发现词表检索在原本 high 可完整重建的素材上可能只配准出极少视角，导致极致档只提取出一帧画面。因此 extreme 默认也改用 exhaustive，把完整配准放在匹配速度之前。

**实现约束**：当前产品不暴露特征匹配高级选项，三个质量档的匹配策略只由后端质量 profile 决定。由于词表检索已从默认策略中移除，代码不再保留词表索引格式失败后的专用重试分支；如果后续重新引入高级匹配选项，应重新评估 COLMAP 版本、词表格式和失败回退。

**备选**：全部档统一 exhaustive（preview 会变慢，不利于快速试错）；统一词表检索（依赖联网下载词表，且本机实测有配准过少风险）；暴露为用户高级选项（更灵活，但当前不保留无 UI 入口的隐藏策略）。
