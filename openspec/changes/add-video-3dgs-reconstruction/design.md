...
- 其他未验证路线不纳入当前有效方案；相关探索保留在单独的废弃变更记录中，避免影响稳定路线的产品契约。
...
- “效果好且基本可用”更依赖逐场景优化，而不是一次性模型推断。
- Nerfstudio/Splatfacto/gsplat 类路线能导出标准 `.ply`，和当前项目契约贴合。
- 当前已经在目标 Windows + NVIDIA 机器上验证稳定路线可产出可用模型，应优先收敛该路线的可靠性和用户体验。

**备选方案**

- 一次性视频到 3D 替代路线：速度快、体验好，但本次探索在目标机器上出现显存压力和质量问题，已移入废弃记录，不作为当前产品路线。
...
- README 和规则文档只把 Windows + NVIDIA RTX 5070 Ti Laptop GPU 12GB 写为视频推理已验证平台；其他平台需要实际验证后再更新。
...
```text
默认质量：快速预览 / 高质量 / 极致
单次任务：可选择自定义，填写目标帧数、训练迭代、输入下采样、COLMAP 匹配方式和图像缓存
默认引擎：自动 / 稳定
显存预算：自动 / 8GB / 12GB / 自定义
保留中间文件：开关
依赖状态：ffmpeg、ffprobe、COLMAP、Nerfstudio/Splatfacto
```
...
视频重建依赖检测包含 `ffmpeg/ffprobe`、Nerfstudio/Splatfacto、COLMAP 等外部命令探测。它们的结果在一次应用进程生命周期内通常不会频繁变化，因此不应在首页加载、打开弹窗或每次提交任务时同步重扫。
...
| 自定义 | 单次任务按素材长度和目标效果调整资源预算 | 用户填写目标帧数、训练迭代、1x/2x/4x 输入下采样、sequential/exhaustive 匹配和 GPU/CPU 图像缓存 |

长视频不再作为固定预设暴露：用户可在自定义中从约 600 帧、35k 迭代、2x 输入下采样、sequential 匹配和 CPU 图像缓存起步，按素材反馈继续收紧或放宽参数。
...
- `auto` 应作为推荐默认；当前等价于稳定路线，保留未来策略切换空间。
- 未验证初始化路线不属于当前有效产品路线，相关经验记录在废弃 OpenSpec 中。

## 风险 / 取舍

- [显存不足导致 OOM] -> 默认串行执行 GPU-heavy 任务；质量档绑定帧预算；检测 OOM 文本并给出降低质量/帧数建议；Settings 提供显存预算。
- [外部依赖安装复杂] -> 依赖检测前置；只展示当前稳定路线实际需要的依赖，避免让用户配置已废弃路线。
...
extreme (360 帧)  exhaustive   与高质量档一致，宁可增加匹配耗时，也优先保证注册完整
custom  (用户填写) sequential/exhaustive 由用户选择；后端限制危险组合，避免高帧数 exhaustive 平方级匹配爆炸
```

**原因**：匹配只影响 COLMAP 阶段中的“特征匹配”子步骤，不影响占总耗时约 80% 的训练；实测 high 档总耗时约 52 分钟、COLMAP 阶段约 8 分钟，换 exhaustive 预计总时长增加约 5~10%，换取显著更高的配准成功率与完整性，性价比高。extreme 帧数翻倍会让 exhaustive 的匹配对数翻 4 倍，但用户实测发现词表检索在原本 high 可完整重建的素材上可能只配准出极少视角，导致极致档只提取出一帧画面。因此 extreme 默认也改用 exhaustive，把完整配准放在匹配速度之前。长视频素材的合理参数与拍摄路径强相关，固定 long 档容易误导；改为自定义后，用户可用 600 帧 + sequential 提高时间覆盖，也可以按房间长度、转弯次数和显存反馈继续调整。

**实现约束**：默认预设的匹配策略由后端质量 profile 决定；自定义入口显式暴露 matching method，并限制高帧数 exhaustive 这类危险组合。由于词表检索已从默认策略中移除，代码不再保留词表索引格式失败后的专用重试分支；如果后续重新引入高级匹配选项，应重新评估 COLMAP 版本、词表格式和失败回退。

**备选**：全部档统一 exhaustive（preview 会变慢，不利于快速试错）；统一词表检索（依赖联网下载词表，且本机实测有配准过少风险）；继续增加 long 固定预设（实现简单，但无法覆盖长视频拍摄差异，且用户容易误以为长视频都有同一组最优参数）。
...
- **可靠性补充**：Nerfstudio/viser 实时查看器是训练观测增强，不是产物生成必需路径。Windows 上 viser client autobuild 可能在检查 yarn 进程时因为 `psutil.NoSuchProcess` 失败；系统应识别这类 viewer 启动失败，清理本次半成品训练目录，并以 `--vis tensorboard` 重试 `ns-train`，继续依赖训练日志推进任务进度。
- **静默阶段心跳**：抽帧/COLMAP（ns-process-data）等步骤常长时间无 stdout 输出，像卡死；后台用一个心跳线程在持续无输出时按间隔打印一条 INFO，提示该步骤仍在运行及已用时长，进程结束即停止。
- **任务计时**：任务进入处理即记录开始时间，前端任务卡片实时显示已处理时长；视频任务在阶段切换与结束时记录各阶段耗时与总耗时，便于排查各环节占用。
...
视频重建失败或取消时，系统除了清理 `jobs/<task_id>` 临时目录外，还应删除该任务预写的 sidecar、缩略图、未完成输出文件，以及 `.video-reconstruction/uploads` 下由拖入上传产生且未被成功模型引用的源视频副本；来自本地相册的原始视频仍只读引用，绝不删除。应用启动时执行一次 best-effort 恢复清理：终止命令行或工作目录明确指向当前 workspace 视频重建目录的 Nerfstudio/COLMAP 残留进程，删除遗留 job 目录，清掉没有完成 `.ply` 引用的预写 sidecar 和孤儿上传缓存。清理失败只记录日志，不阻塞应用启动。
