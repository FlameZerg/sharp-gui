...
为 Sharp GUI 增加本地照片相册图库能力，使用户能够配置多个本地照片目录、按相册浏览照片、以高性能瀑布流查看缩略图、预览和下载原图，并将单张或多张照片加入现有 3D 生成队列。该能力必须同时兼容 Windows、Linux 和 macOS 部署端。
...
The system SHALL list configured photo directories as albums with a display name, cover thumbnail when available, photo count or scan status, and updated timestamp.
...
The photo gallery UI SHALL keep browsing responsive for large albums by using paginated data loading, thumbnail assets, stable layout sizing, and windowed or otherwise bounded rendering.

#### Scenario: Album contains at least 1000 photos
- **WHEN** the user opens an album containing at least 1000 photos
- **THEN** the UI SHALL become interactive without waiting for all photos to be fetched, decoded, or rendered
...
The photo gallery SHALL allow users to sort photos by common file properties using localized labels.
