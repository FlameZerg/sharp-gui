...
本地媒体图库和局域网门禁相关控件应优先复用 `components/common/SelectMenu`、`ConfirmDialog`、`TextInputDialog` 等通用玻璃态组件，避免浏览器原生 `select` / `prompt` / `confirm` / `alert` 造成视觉割裂。
...
- 视频卡片可使用 poster、播放图标、时长和规格标签；poster 不可用时使用克制 fallback，不要在网格中放真实 video 元素批量加载。
