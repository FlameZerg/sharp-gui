...
**🎥 视频 3DGS 重建（实验）**：在 Windows + NVIDIA RTX 5070 Ti Laptop GPU 环境中，已验证本地视频可通过 Nerfstudio/Splatfacto 稳定路线生成 `.ply/.spz` 模型；支持质量档、focused cleanup、视频封面缩略图、原视频回看和视频模型预览坐标适配。
...
Windows RTX 50 / 主流 NVIDIA 用户可直接下载完整便携包，无需手动配置 Python、PyTorch 或模型缓存。

固定网盘入口：[点击打开网盘文件夹](https://pan.quark.cn/s/94f4acaada40)

网盘文件夹会持续更新到最新版本，请按显卡选择：

- **RTX 50 系列**：下载 `cu128-rtx50` 包
- **RTX 50 以下主流 NVIDIA**：下载 `cu126-mainstream` 包

下载 ZIP 和同名 `.sha256.txt` 后先校验 SHA256，解压后双击 `portable-run.bat` 启动。

> 💡 完整便携包目前面向 NVIDIA GPU，不提供纯 CPU 包。
...
- 🎥 **安装视频重建环境（Windows NVIDIA，实验）** - 可安装或复用 `.video-reconstruction-env`、CUDA/PyTorch、Nerfstudio/Splatfacto、gsplat、COLMAP 和 `ffmpeg/ffprobe`
...
2. **选择参数** - 弹窗中可选择模式（自动 / 物品 / 环境）、质量（快速预览 / 高质量 / 极致）和引擎（自动 / 稳定 / 实验）
...
- **默认引擎**：自动。π³ 前馈引擎就绪时优先用它一次推理估计相机位姿（替换最慢的 COLMAP 几何阶段，分钟级），未配置或失败时自动回退稳定 Nerfstudio/Splatfacto 路线
...
- **默认质量**：快速预览 / 高质量 / 极致，对应不同帧数、训练迭代和输入分辨率预算
- **默认引擎**：自动 / 稳定 / 实验；实验引擎为 π³（Pi3）前馈引擎，是可选增强，缺失时不影响自动或稳定路线
- **π³ 前馈引擎（实验，可选）**：以一次神经网络推理估计相机位姿，替换最慢的 COLMAP 几何阶段。**π³ 代码为 BSD-3，但其模型权重为 CC BY-NC 4.0（仅限非商业研究/教育）**，DINOv2 backbone 为 Apache 2.0。为遵守许可，**本项目默认不打包、不自动下载权重**，需用户自行下载并接受其许可后放置到约定目录（默认 `.feedforward-weights/`，可用环境变量 `SHARP_GUI_FEEDFORWARD_WEIGHTS_DIR` 覆盖）。第一验证平台为 **Windows + NVIDIA RTX 5070 Ti Laptop GPU 12GB**
...
以下说明面向有动手能力的用户和 AI Agent，覆盖从零搭建所有视频重建路线（稳定 COLMAP + 实验前馈）的依赖。当前唯一已验证平台为 **Windows + NVIDIA GPU（RTX 5070 Ti Laptop 12GB）**。
...
#### 4. 安装实验路线依赖（π³ 前馈引擎，可选）

此步骤完全可选——不安装时 `auto` 和 `stable` 引擎正常工作，只是不能使用前馈加速。

##### 4a. 安装 π³ 推理包

```bash
# 在已激活的 .video-reconstruction-env 中
git clone https://github.com/yyfz/Pi3.git /path/to/Pi3
cd /path/to/Pi3
pip install -r requirements.txt
# 确保 pi3 包可导入（Pi3 仓库会安装为本地包）
pip install -e .
```

> 验证：`python -c "from pi3.models.pi3x import Pi3X; print('ok')"`

##### 4b. 下载 π³ 模型权重

> ⚠️ **许可声明**：π³ 模型权重采用 **CC BY-NC 4.0** 许可（仅限非商业研究/教育用途）。下载即表示你接受该许可条款。本项目绝不自动下载或分发这些权重。

```bash
# 创建约定权重目录（在 sharp-gui 项目根目录）
mkdir -p .feedforward-weights/pi3x

# 下载 Pi3X 权重（推荐，约 2.1 GB）
wget -O .feedforward-weights/pi3x/model.safetensors \
  https://huggingface.co/yyfz233/Pi3X/resolve/main/model.safetensors

# 或下载原始 Pi3 权重（约 2.1 GB）
...
# Pillow（图片加载，一般已被 torchvision 依赖安装）
pip install Pillow
```
...
| **π³ 前馈引擎** (experimental) | ✅ Available（或 ⚠️ Missing 如未安装） | pi3-engine（包可导入）, pi3-weights（权重就绪） |
...
| Pi3 (π³) | main 分支 | `pi3.models.pi3x.Pi3X` 可导入即可 |
| safetensors | 0.4+ | 加载 .safetensors 权重 |
...
│   └── Lib/ 或 lib/              #   → torch, nerfstudio, gsplat, pi3 等包
├── .feedforward-weights/          # π³ 权重目录（已被 .gitignore，用户自行下载）
│   └── pi3x/
│       └── model.safetensors     #   Pi3X 权重（~2.1 GB，CC BY-NC 4.0）
...
> 💡 `.video-reconstruction-env` 和 `.feedforward-weights` 均已在 `.gitignore` 中排除，不会被提交到仓库。
...
| **AI 引擎**  | Apple ML-Sharp (PyTorch, gsplat)；视频重建实验路线使用 Nerfstudio/Splatfacto + COLMAP/ffmpeg |
