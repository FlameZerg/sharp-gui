...
>
> 🎥 **视频重建环境需单独确认**：常规安装会先保证主程序和图片 3D 推理可用；如果要从视频生成 3D 模型，请按 [视频重建环境手动搭建指南](#视频重建环境手动搭建指南) 安装或复用 `.video-reconstruction-env`、Nerfstudio/Splatfacto、COLMAP 和 `ffmpeg/ffprobe`。
>
