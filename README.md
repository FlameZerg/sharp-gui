...
| **🗂️ 本地照片图库** | 配置多个本地目录作为相册，瀑布流浏览缩略图，打开原图预览/下载，并支持单张或多选批量转换为 3D 照片       |
...
- **本地相册** - 在「照片」视图中添加多个目录，每个目录以相册形式展示
- **可调瀑布流** - 支持修改时间/创建时间/名称/大小排序，桌面端滑块调列数，移动端支持双指捏合调整密度
- **队列管理** - 待处理任务支持删除/取消
...
Sharp GUI 提供可选的局域网门禁。首次启动或本机尚未完整配置门禁时，会提示 owner 设置访问码；用户也可以稍后设置或选择不再提示。

- 门禁开启后，模型列表、缩略图、原图、照片相册、下载、导出和 `/files/*` 工作区资源需要局域网设备先输入访问码；解锁后浏览器会保存 HttpOnly Cookie 会话。
- 门禁关闭时，同一局域网内能访问端口的设备可直接浏览和下载私有内容；删除、设置、重启、目录管理等 owner-only 操作仍仅限 `localhost` / `127.0.0.1`。
- 远程设备即使输入了访问码，也默认只有浏览、预览、下载和导出权限；生成任务和照片转 3D 默认仍是 owner-only。
- 如需允许已解锁的远程设备提交生成任务，可在 Settings 的“局域网门禁”中开启“远程生成”；关闭门禁会同时收回远程生成权限。
- HTTPS 负责传输加密和浏览器传感器能力，访问码负责访问资格；在局域网共享端口时建议两者都启用。
...
frontend/
├── 📁 src/
│   ├── 📁 api/               # API 客户端 (gallery, photoGallery, tasks, settings)
│   ├── 📁 components/
