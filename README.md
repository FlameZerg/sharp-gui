...
- 🎥 **安装视频重建环境（Windows NVIDIA）** - 可安装或复用 `.video-reconstruction-env`、CUDA/PyTorch、Nerfstudio/Splatfacto、gsplat、COLMAP 和 `ffmpeg/ffprobe`
...
2. **选择参数** - 弹窗中可选择模式（自动 / 物品 / 环境）、质量（快速预览 / 高质量 / 极致）和引擎（自动 / 稳定）
...
以下说明面向有动手能力的用户和 AI Agent，覆盖从零搭建稳定视频重建路线（COLMAP + Nerfstudio/Splatfacto）的依赖。当前唯一已验证平台为 **Windows + NVIDIA GPU（RTX 5070 Ti Laptop 12GB）**。
