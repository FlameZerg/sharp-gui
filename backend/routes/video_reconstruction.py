...
@bp.route("/api/video-reconstructions/upload", methods=["POST"])
def upload_video_reconstruction():
    """从拖入的本地视频文件创建 3DGS 重建任务。"""
    uploaded_file = request.files.get("file")
    request_data, error_payload, status_code = video_reconstruction.validate_upload_request(
        request.form,
        uploaded_file,
    )
    if error_payload:
    ...
    paths = current_app.config["PATH_CONTEXT"]
    task_payload, build_error, build_status = video_reconstruction.build_uploaded_video_task(
        paths,
        request_data,
        uploaded_file,
    )
...
def video_reconstruction_status():
    ...
    return jsonify({
        ...
        "dependencies": video_reconstruction.get_dependency_status(
