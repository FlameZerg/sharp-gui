...
def build_photo_album_response(paths, album):
    """构建相册响应数据。"""
    photos, scan_status, error = scan_photo_album(paths, album)
...
def list_album_photos(paths, album_id, sort, cursor_value, limit_value):
    ...
    try:
        ...
        if added_count == 0:
            try:
                ...
                pass
            return None, {"error": "No downloadable photos found", "failed": failed}, 404
